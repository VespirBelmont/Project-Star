...
#	if state == "WallSlide": #If the character is wall sliding
#		velocity.y = -wall_jump_settings.JumpForce #Set the launch force to the wall jump setting instead of the character launch force
#		velocity.x = wall_jump_settings.PushOff * (direction.x * -1) #Do a walljump which means moving horizontally
